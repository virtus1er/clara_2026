...
struct ConscienceConfig {
    ...
    double trauma_alert_threshold = 0.8; // Seuil pour déclencher une alerte trauma
    ...
    double beta_feedback = 0.2;      // β : Pondération feedback externe (legacy)
    double beta_memory = 0.15;       // β : Pondération feedback dans calcul conscience
    ...
    double sentiment_smoothing = 0.1;    // α : Lissage EMA du sentiment
    double sentiment_window_seconds = 300.0; // Fenêtre historique (5 minutes)
    ...
    double min_consciousness_threshold = 0.1; // Seuil minimum pour état "actif"
 ...
 */
struct TraumaState {
    std::string id;                      // Identifiant unique du trauma
    ...
    std::string source;                  // Source/origine du trauma
    uint64_t source_memory_id = 0;       // ID du souvenir source dans le graphe
    bool is_active = false;              // État d'activation

    [[nodiscard]] bool isActive() const { return is_active && type != TraumaType::NONE && intensity > 0.1; }
...
struct MemoryActivation {
    ...
    // Champs pour compatibilité avec ConscienceEngine
    double activation_strength = 0.0;   // Force d'activation globale
    double emotional_resonance = 0.0;   // Résonance émotionnelle
...
struct EnvironmentState {
    ...
    [[nodiscard]] double score() const { return computeScore(); }
};
...
struct FeedbackState {
    ...
    // Alias pour compatibilité
    [[nodiscard]] double combined() const { return computeScore(); }
};

/**
 * État de conscience et sentiment calculé
 */
struct ConscienceSentimentState {
    double consciousness_level = 0.0;    // Ct : niveau de conscience [0, 1]
    double sentiment = 0.0;              // Ft : sentiment global [-1, +1]
    double wisdom = 1.0;                 // Wt : sagesse actuelle

    // Contributions détaillées
    double emotional_contribution = 0.0;
    double memory_contribution = 0.0;
    double trauma_contribution = 0.0;
    double feedback_contribution = 0.0;
    double environment_contribution = 0.0;

    // Métriques additionnelles
    double sentiment_moving_average = 0.0;  // EMA du sentiment
    std::string dominant_state = "neutral"; // positive/negative/neutral/dormant
...
using ConscienceUpdateCallback = std::function<void(const ConscienceSentimentState&)>;
using TraumaAlertCallback = std::function<void(const TraumaState&)>;
using MLTModulationCallback = std::function<void(const std::array<double, 24>&)>;
